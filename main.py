...
def export_orders_pdf(admin_username: str):
    ...
    # Przygotuj dane - 7 kolumn (bez ID zamówienia)
    data = []
    headers = [
        "Nazwisko Imie",
        "RCP",
    ...
    for order in orders:
        ...
        # Przygotuj wiersz danych (bez ID zamówienia)
        row = [
            user_code,
            order["username"],
            order["week"],
    ...
    # Szerokości kolumn (7 kolumn)
    col_widths = [100, 40, 45]  # Kod, Użytkownik, Tydzień
    col_widths.extend([95, 95, 95, 95, 95])  # 5 dni tygodnia
